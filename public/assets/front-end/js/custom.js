...
function checkoutFromCartList() {
    ...
    // Adjust the URL to match your Laravel route names
    let orderNoteUrl = $('#route-order-note').data('url');
    let gstNumberUrl = $('#route-gst-number').data('url');

    $.ajax({
        type: 'POST',
        url: orderNoteUrl, // Endpoint for saving order note
        data: {
        ...
        success: function(orderNoteResponse) {
            if (orderNoteResponse.status === 0) {
                orderNoteResponse.message.forEach(function(message) {
                    toastr.error(message);
                });
                $('#loading').hide(); // Hide loading indicator on error
            } else {
                // On successful order note save, proceed to save GST number
                $.ajax({
                    type: 'POST',
                    url: gstNumberUrl, // Endpoint for saving GST number
                    data: {
                        _token: $('meta[name="_token"]').attr('content'),
                        gst_number: gstNumber,
                    },
                    success: function(gstNumberResponse) {
                        if (gstNumberResponse.status === 0) {
                            gstNumberResponse.message.forEach(function(message) {
                                toastr.error(message);
                            });
                        } else {
                            // Both order note and GST number saved successfully
                            location.href = gstNumberResponse.redirect ? gstNumberResponse.redirect : $('#route-checkout-details').data('url');
                        }
                    },
                    complete: function() {
                        $('#loading').hide(); // Hide loading indicator on GST number save
                    },
                });
            }
        },
        error: function(xhr, status, error) {
            toastr.error('Error occurred while saving order note.');
            $('#loading').hide(); // Hide loading indicator on error
        },
    });
}
